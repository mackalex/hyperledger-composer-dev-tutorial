...
describe('Starting tests for tutorial-network-angular', function() {
  ...
  it('website title should be tutorial-network-angular', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('tutorial-network-angular');
    })
  });
  ...
  it('navbar-brand should be tutorial-network-angular',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('tutorial-network-angular');
    });
  });
